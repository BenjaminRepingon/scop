...
/*   Updated: 2015/06/03 13:45:46 by rbenjami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	gen_normals_loop(t_object *object, VEC3 *buffer)
{
	...
	i = 0;
	while (i < object->obj->indices.size)
	{
...
static void	gen_normals(t_object *object, VEC3 *buffer)
{
	...
	i = 0;
	while (i < object->obj->vertex.size)
...
VEC3		*get_normals_buffer(t_object *object)
{
	...
	if ((buffer = ft_memalloc(sizeof(VEC3) * object->obj->vertex.size)) == NULL)
		return (NULL);
